...
class InternetConnectivityCubit extends Cubit<InternetConnectivityState> {
  InternetConnectivityCubit() : super(const InternetConnectivityState(status: ConnectivityStatus.disconnected));
  ...
  void _updateConnectivityStatus(List<ConnectivityResult> result) {
    if(result.contains(ConnectivityResult.none)) {
      emit(const InternetConnectivityState(status: ConnectivityStatus.disconnected));
    } else if(result.contains(ConnectivityResult.mobile) || result.contains(ConnectivityResult.wifi)) {
      emit(const InternetConnectivityState(status: ConnectivityStatus.connected));
    }
  }
